...
{
    public partial class JeuFacile : Form // Hérite de Form pour être utilisé comme formulaire Windows Forms
    {
        ...
        public JeuFacile(int hauteur, int largeur)
        {
            ...
            int xOffset = -60; // Décalage horizontal désiré
                ...
                if (fantomes[i] == pacman)
                {
                    ...
                    else
                    {
                        ...
                        if (vies <= 0)
                        {
                            minuterie.Stop();
                            AfficherGameOver();
                            FermerApplicationApresDelai(); // Attendre 5 secondes avant de fermer l'application
